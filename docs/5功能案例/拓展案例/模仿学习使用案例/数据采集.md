...
机器人搭载了两块`NUC`，`NVIDIA Jetson AGX Orin`作为**上位机**，`摩方i9-13900`作为**下位机**。详细参数见[参考资料](#reference-material)。模型训练部署框架（`kuavo-il-opensource`）都在**上位机**上。所以进入采集平台前要获得要获得**上位机**在局域网中的ip。
