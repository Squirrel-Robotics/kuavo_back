...
`env.py`所在的路径为`~/kuavo_il/kuavo/kuavo_3deploy/env.py`，env.py主要提供类KuavoEnv，用于获取传感器观测数据（关节信息、传感器参数、图像信息）和控制机器人的关节运动。
...
`eval.py`所在的路径为`~/kuavo_il/kuavo/kuavo_3deploy/eval.py`,读取配置文件，根据配置文件加载`lerobot`模型框架，实例化模型策略`policy`
