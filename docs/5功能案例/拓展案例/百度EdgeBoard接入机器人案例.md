...
#### 案例流程逻辑
  - 参考本文档[(二)百度EdgeBoard环境配置](#二百度EdgeBoard环境配置) 配置百度EdgeBoard开发板环境
  - 参考本文档[(三)百度EdgeBoard模型部署](#三百度EdgeBoard模型部署) 部署yolo目标检测模型
  - 参考本文档[(四)ROS通信系统配置](#四ros通信系统配置)的部分 配置机器人上下位机及百度EdgeBoard开发板间的通信
  - 参考本文档[(五)启动流程示例](#五启动流程示例)的部分 启动本案例程序
