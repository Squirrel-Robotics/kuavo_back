...
```bash
git clone https://www.lejuhub.com/tianchu/kuavo-manip-open.git
cd kuavo-manip-open
# 切换分支（重要）
...
- 在 `kuavo-ros/src/kuavo-ros-control/src/demo/grab_box/cfg/` 下新建 `kuavo_v60/` 目录
...
- **切换控制模式**：下位机正常启动后需要切换轮臂控制模式。先运行脚本：
  - `kuavo-ros-control-lb/src/demo/wheel_arm_demo/wheel_control_mode_swither.py`
