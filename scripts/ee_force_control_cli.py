...
class EEForceControlCLI:
    def __init__(self):
        ...
        # 创建两个Publisher，分别对应左右手
        self.pub_left = rospy.Publisher('/desired_ee_force/left', WrenchStamped, queue_size=10)
        self.pub_right = rospy.Publisher('/desired_ee_force/right', WrenchStamped, queue_size=10)
        self.gravity = 9.8  # 重力加速度，用于 kg 转 N
        self.max_force_kg = 80.0 / 9.8  # 最大力值60N转换为kg（约6.12kg）
    ...
    def publish_force(self, hand, force_x_kg, force_y_kg, force_z_kg):
        ...
        # 将 kg 转换为 N: 1kg = 9.8N
        msg.wrench.force.x = force_x_kg * self.gravity
        msg.wrench.force.y = force_y_kg * self.gravity
        msg.wrench.force.z = force_z_kg * self.gravity
        ...
        if hand == 'left':
            ...
            rospy.loginfo(f"已发布左手期望力: x={force_x_kg:.2f}kg ({msg.wrench.force.x:.2f}N), "
                         f"y={force_y_kg:.2f}kg ({msg.wrench.force.y:.2f}N), "
                         f"z={force_z_kg:.2f}kg ({msg.wrench.force.z:.2f}N)")
        elif hand == 'right':
            self.pub_right.publish(msg)
            rospy.loginfo(f"已发布右手期望力: x={force_x_kg:.2f}kg ({msg.wrench.force.x:.2f}N), "
                         f"y={force_y_kg:.2f}kg ({msg.wrench.force.y:.2f}N), "
                         f"z={force_z_kg:.2f}kg ({msg.wrench.force.z:.2f}N)")
        elif hand == 'both':
            self.pub_left.publish(msg)
            self.pub_right.publish(msg)
            rospy.loginfo(f"已发布双手期望力: x={force_x_kg:.2f}kg ({msg.wrench.force.x:.2f}N), "
                         f"y={force_y_kg:.2f}kg ({msg.wrench.force.y:.2f}N), "
                         f"z={force_z_kg:.2f}kg ({msg.wrench.force.z:.2f}N)")
    ...
    def input_force_option(self):
        ...
        while True:
            ...
            if confirm == 'y':
                if hand == 'both':
                    self.publish_force('left', force_x, force_y, force_z)
                    self.publish_force('right', force_x, force_y, force_z)
                else:
